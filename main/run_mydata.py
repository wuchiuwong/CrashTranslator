...
def get_app_info(app_idx, dataset_dir="../Data/Myda", debug=False):
    app_dict = {}
    for app_name in os.listdir(dataset_dir + "/apps"):
        if "24.5" in app_name:
            continue
        app_dict.setdefault(int(app_name.split("_")[0]), app_name)
    ...
    if len(trace_info["trace_actis"]) > 0:
        ...
        # print(trace_eles_class[0])
        # print(trace_eles_class[0].split(".")[-1].split("$")[0])
